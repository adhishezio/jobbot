...
def _confirm_address(exec_id, _resume_url_path, street, plz):
    """Persist the confirmed address and resume the n8n Wait node."""
    try:
        execute(
            """
        ...
        if response.status_code == 200:
            st.success("Address confirmed. The pipeline is continuing now.")
            return True

        st.error(f"n8n returned status {response.status_code}: {response.text[:200]}")
        return False
    except requests.exceptions.Timeout:
        st.error("n8n did not respond in time while resuming the workflow.")
        st.stop()
        return False
    except Exception as exc:
        st.error(f"Could not confirm the address: {exc}")
        st.stop()
        return False
                ...
                with col1:
                    if st.button("Confirm", key=f"confirm_{conf['execution_id']}"):
                        ...
                        if success:
                            ...
                            st.rerun()
                with col2:
                    if st.button("Edit", key=f"edit_{conf['execution_id']}"):
                        st.session_state[f"editing_{conf['execution_id']}"] = True
            else:
                st.error("Address not found automatically. Please enter it manually.")
                st.session_state[f"editing_{conf['execution_id']}"] = True
            ...
            if st.session_state.get(f"editing_{conf['execution_id']}", False):
                ...
                if st.button("Submit", key=f"submit_{conf['execution_id']}"):
                    if new_street and new_plz:
                        ...
                        st.error("Both address fields are required.")
...
def show_cover_letter_badge():
    ...
    if count > 0:
        ...
        st.sidebar.success(f"{count} cover letter{suffix} generated today")
